...
##install.packages(c("plyr", "httr", "jsonlite", "XML", "stringr", "openair", "chron", "lubridate"))
...
last_timestamp<-t(as.data.frame(lapply(dt_list,"[", 1, 1)))
...
for(i in 1:length(dt_list_new_values)){
  ...
  if(any(pos_eq)){
    dt_sum_list[[i]]<-viejos_dt[,]
    
    
  }else{
    cc<- rbind(nuevos_dt[1:pos_eq-1,],viejos_dt[,])
    dt_sum_list[[disp$Disp[i]]]<- cc
  }
}
